...
+ggtitle("Verrijking GO-proces bij reumatoïde artritis")
...
#Extra visualisatie KEGG

library(clusterProfiler)
library(org.Hs.eg.db)
gene_df <- bitr(genen_list, fromType = "SYMBOL", toType = "ENTREZID",OrgDb = org.Hs.eg.db)
head(gene_df)
geneList <- resultaten$log2FoldChange
names(geneList) <- rownames(resultaten)
geneList <- geneList[match(gene_df$SYMBOL, names(geneList))]
names(geneList) <- gene_df$ENTREZID
library(pathview)

pathview(gene.data = geneList, pathway.id = "hsa05323", species = "hsa")
