...
De ruwe sequencing reads werden eerst gemapt naar het humane referentiegenoom GRCh38.p14 (GCF_000001405.40), beschikbaar via NCBI:https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_000001405.40/ met behulp van het Rsubread (versie 2.24.0) package in R. Vervolgens werd met featureCounts het aantal reads per gen bepaald, wat resulteerde in een count matrix. 

Differentiële genexpressie tussen de controlegroep en de RA-groep werd geanalyseerd met behulp van het DESeq2 (versie 1.50.2) package. Genen met een aangepaste p-waarde (padj < 0,05) werden beschouwd als significant differentieel tot expressie komend. Deze significante genen werden vervolgens gebruikt als input voor de GO-analyse en KEGG-pathwayanalyse. Voor de GO-analyse werden de geselecteerde genen omgezet naar ENTREZ-ID's met behulp van het package org.Hs.eg.db (versie 3.22.0). Vervolgens werd met clusterProfiler (versie 4.18.4) een verrijkingsanalyse uitgevoerd om biologische processen te identificeren die geassocieerd zijn met de gevonden genexpressieveranderingen. Hiermee konden genen worden gegroepeerd op basis van hun betrokkenheid bij biologische processen. Daarnaast werd een KEGG-pathway analyse uitgevoerd om inzicht te krijgen in welke signaalroutes en pathways betrokken zijn bij reuma. Voor verdere analyse werd specifiek gekeken naar pathway hsa05323 (Rheumatoïde Arthritis pathway). Deze pathway werd geselecteerd omdat deze direct gerelateerd is aan de pathogenese van reuma en inzicht geeft in de betrokken immuun- en ontstekingsprocessen [2]. Het flowschema van de analyse is weergegeven in figuur 1. 
<p align="center">
